      ...
      return {
          ...
          link: function postLink(scope, element, attrs) {
              ...
              scope.$watch('colorScale.value.domain()', function(domain) {
                  console.log("colorScale.value.domain()")
          ...
          link: function postLink(scope, element, attrs) {
              ...
              function message() {
                  ...
                  scope.messageText = hasInvalidType ?
                      "You should only use " + scope.types.map(function(d) {
                          return d.name.toLowerCase() + "s";
                      }).join(" or ") + " here" :
                      "Drag " + scope.types.map(function(d) {
                          return d.name.toLowerCase() + "s";
                      }).join(", ") + " here";
          ...
          link: function postLink(scope, element, attrs) {
              ...
              var downloadData = function() {
                  ...
                  saveAs(blob, (scope.filename || element.find('input').attr("placeholder")) + ".json")
							...
							var downloadInteractiveModel = function() {

								var raw_config = {
									"data": raw.parser(scope.data),
									"dimensions": {
										"Clusters": {
											"key": "Movie",
											"type": "String"
								...
								console.log(JSON.stringify(raw_config));

								var zip = new JSZip();
								$.get("http://localhost:5000/js/module.js", function(data) {
									zip.file("js/module.js", data);
									$.get("http://localhost:5000/raw_model.html", function(data) {
										zip.file("index.html", data);
										$.get("http://localhost:5000/charts/clusterForce.js", function(data) {
											zip.file("charts/clusterForce.js", data);
											$.get("http://localhost:5000/lib/raw.js", function(data) {
												zip.file("lib/raw.js", data);

												zip.file("raw_config.json", JSON.stringify(raw_config));
              ...
              scope.modes = [
                  ...
                  {
                      label: 'Interactive model (zip)',
                      download: downloadInteractiveModel
                  }
              ]
          }
      };
