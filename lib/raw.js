...
!function(exports){
  ...
  raw.legend = function() {
    ...
    function legend(_selection) {

        _selection.each(function(){

            var w = _selection.attr('width')
            var h = _selection.attr('height')
            var t = getTransformation(_selection.select('g').attr("transform"));

            console.log(w,h,t);

            var legendContainer = _selection.append('g')
              .attr("transform", "translate(" + ( w - legendWidth ) + ","+ t.y +")");
            ...
            legendContainer.append('rect')
              ...
              .attr('y',0)
            //draw size scale
            if (legendSize && legendSize.dimension.value.length) {

                legendContainer.append("g")
                    .attr("class", "legendSize")
                    .attr("transform", "translate(0,10)");

                var d3LegendSize = d3.legendSize()
                    .scale(legendSize.scale)
                    .cells(legendSize.extent)
                    //.cellFilter(function(d){return d.data > 0})
                    .shape('circle')
                    .title(legendSize.dimension.value[0].key)
                    .shapePadding(legendSize.scale.range()[1]);
            ...
            //draw color scale
            if (legendColor && legendColor.dimension.value.length) {

                var legendColorHeight = legendContainer.select('.legendSize').empty()?0:legendContainer.select('.legendSize').node().getBBox().height

                legendContainer.append("g")
                    .attr("class", "legendColor")
                    .attr("transform", "translate(0," + (legendColorHeight + legendPadding) + ")");

                var d3legendColor = d3.legendColor()
                    .shapePadding(shapePadding)
                    .title(legendColor.dimension.value[0].key)
    ...
    legend.addColor = function(_dimension, _scale) {
        if(!arguments.length) return legendColor;

        legendColor = {'dimension': _dimension,
                        'scale': _scale};
        return legend;
    }

    legend.addSize = function(_dimension, _scale, _extent) {

        if(!arguments.length) return legendSize;
        legendSize = {'dimension': _dimension,
                        'scale': _scale,
                        'extent': _extent};
        return legend;
    }
