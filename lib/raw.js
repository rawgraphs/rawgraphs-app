!function (exports){
  ...
  var raw = { models : d3.map(), charts : d3.map() };
  ...
  raw.parser = function (delimiter) {
    ...
    function detectDelimiter(string){
      ...
      for (var characterCount in header) {

        character = header[characterCount];

        switch(character) {

          case '"':
            if (quoted) {
              if (header[characterCount+1] != '"') quoted = false;
              else characterCount++;
            }
