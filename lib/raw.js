...
!function (exports){
      ...
      while (matches = re.exec(string)){
        try {
          var strMatchedDelimiter = matches[1],
              strMatchedValue = matches[2] ? matches[2].replace(new RegExp( "\"\"", "g" ), "\"" ) : matches[3];

          if (strMatchedDelimiter.length && strMatchedDelimiter != delimiter ) rows.push([]);
  ...
  var chart_option_number = function() {
    ...
    var option = chart_option().type('number'),
        fitToWidth = false;
  ...
  // Select option
  var chart_option_select = function() {

    var option = chart_option().type('select'),
        options = [];

    option.options = function(_){
  ...
  var chart_option_checkbox = function() {
    return chart_option().type('checkbox');
  }

  // Color option
  var chart_option_color = function() {
    var option = chart_option().type('color'),
        domain = [],
  ...
  raw.chart = function (id) { 
    ...
    chart.select = function(id){
      var id = id || options.size();
      var option = chart_option_select();
      options.set(id, option);
      return option;
    }
