(function() {
	...
	chart.draw((selection, data) => {
		...
		// define colors
		colors.domain(data.links, function(d) { return d.source.group + " - " + d.source.name; });
		...
		nested
			.forEach(function(d) {
				...
				var y = (height() - d3.sum(d.values, function(n) { return n.dy + sankey.nodePadding(); })) / 2 + sankey.nodePadding() / 2;
		...
		var link = g.append("g")
			...
			.style("stroke", function(d){return colors()(d.source.group + " - " + d.source.name); })
		...
		node.append("text")
			...
			.attr("y", function(d) { return d.y0 + d.dy / 2; })
			.attr("dy", "0.35em")
			.attr("text-anchor", "end")
			.text(function(d) { return d.name; })
			.filter(function(d) { return d.x0 < nodeWidth() })
