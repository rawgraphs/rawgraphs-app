...
function ChartSelector({
  ...
  const charts = filter === 'All charts' ? availableCharts : availableCharts.filter(d => d.metadata.category === filter);
        ...
        <Col className="text-right">
          ...
          <Dropdown className="d-inline-block ml-2 raw-dropdown">
            ...
            <Dropdown.Menu>
              ...
              {
                uniq(availableCharts.map(d => d.metadata.category)).map(d => {
        ...
        <Col xs={3} className="pt-3">
          {currentChart && (
            <Card>
              ...
              <Card.Body>
                <Card.Title className="m-0"><h2 className="m-0">{currentChart.metadata.name}</h2></Card.Title>
                <Card.Subtitle className="m-0"><h4 className="mb-2">{currentChart.metadata.category}</h4></Card.Subtitle>
                <Card.Text>
                ...
                <Col xs={4} key={'chart-' + i} className={`p-3`}>
                  <Card onClick={() => { setCurrentChart(d) }} className={`flex-row h-100 cursor-pointer ${d === currentChart ? 'active' : ''}`}>
                    ...
                    <Card.Body className="w-75 px-2 py-3">
                      ...
                      <Card.Subtitle className="m-0"><h4 className="m-0">{d.metadata.category}</h4></Card.Subtitle>
                    </Card.Body>
                  </Card>
                </Col>
              )
