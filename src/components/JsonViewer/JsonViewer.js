...
const JsonViewerRecursive = ({ contextName, nestingLevel, context, selectFilter, onSelect }) => {
  ...
  if (contextType === "object" && contextType !== null) {
    ...
    return (
      ...
      >
        ...
        {context === null && (
          <span className="scalar-value">null</span>
        )}
        {context !== null && (
          <>
            {(contextType === "bigint" || contextType === "number") && (
              <span className="scalar-value scalar-value-numeric">{context.toString()}</span>
            )}
            {contextType === "string" && (
              <span className="scalar-value scalar-value-string">"{context.toString()}"</span>
              )}
            {contextType === "boolean" && (
              <span className="scalar-value scalar-value-bool">"{context ? "true" : "false"}"</span>
            )}
            {contextType === "undefined" && (
              <span className="scalar-value scalar-value-undefined">undefined</span>
            )}
          </>
        )}
      </div>
